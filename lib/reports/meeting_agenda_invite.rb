...
class MeetingAgendaReport < Prawn::Document
  ...
  def to_pdf(object, contact, participation_form, outgoing_code)
    set_font_families
    ...
    # Информация о регистрации письма (для e-mail)
    print_outgoing_code(outgoing_code)
    move_down 5
...
#    move_down 5
    ...
    print_agenda_fields(object, contact, participation_form)
    move_down 10
...
private
  ...
  def print_outgoing_code(outgoing_code)
    text(
      l(:label_contact_invite_outgoing_code,
        code: outgoing_code,
        date: format_date(Date.today)),
      style: :bold, size: 22, align: :center
    )
    text(
      l(:label_contact_invite_incoming_code),
      style: :bold, size: 22, align: :center
    )
  end
...
#        t.cells.align = :right
  ...
  def print_agenda_fields(agenda, contact, participation_form)
    text(l(:label_dear_contact, contact: contact.to_s), style: :bold, size: 22, align: :center)
    move_down 10

    text(l(:label_contact_invite_info, meet_on: format_date(agenda.meet_on), start_time: format_time(agenda.start_time, false), end_time: format_time(agenda.end_time, false), subject: agenda.subject), size: 10, inline_format: true)
    text(l(:label_contact_invite_address, address: agenda.is_external? : agenda.address : agenda.meeting_company.fact_address), size: 10, inline_format: true)
    move_down 10

    text(l(:message_participation_form_for_select)[participation_form], size: 10, inline_format: true)
  end
