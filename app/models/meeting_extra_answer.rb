class MeetingExtraAnswer < ActiveRecord::Base
  ...
  validates_presence_of :user_id, :description, :start_date, :due_date, :meeting_question_name
  validates_presence_of :reporter_id, if: -> { self.reporter_id_is_contact? }
  validates_presence_of :external_reporter_id, unless: -> { self.reporter_id_is_contact? }
